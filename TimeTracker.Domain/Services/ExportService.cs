...
public class ExportService : IExportService
{
    ...
    public async Task<Result<ExportTrackingsResponse>> Handle(
        ExportTrackingsRequest request,
    ...
    )
    {
        ...
        return Result.Ok<ExportTrackingsResponse>(new ExportTrackingsResponse());
    }

    private async IAsyncEnumerable<TrackingDto> GetTrackings(
        IEnumerable<Guid> trackingIds,
    ...
    )
    {
        foreach (var trackingId in trackingIds)
        {
            var response = await _mediator.Send(
                new GetTrackingRequest(trackingId),
                cancellationToken
            );
            if (response.Failure)
            {
            ...
            yield return response.Value.Tracking;
        }
    }

    private static void CreateExcelFile(
        IReadOnlyList<TrackingDto> trackings,
